...
/** Participantes de un curso con sus roles, sin recortes.
 *
 ...
 *
 *  Distinta de `getEnrolledUsers`, que acota a docentes, tope de 200 y un
 *  `userfields` sin `username`. Esta devuelve el curso completo. */
export async function getEnrolledUsersWithRoles(
  moodleUrl: string,
  token: string,
  courseId: number,
...
export async function getEnrolledUsersByRole(
  ...
  const users = await getEnrolledUsersWithRoles(moodleUrl, token, courseId);
