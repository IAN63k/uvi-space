...
import type { MoodleCategory, MoodleCourse, ValidationRules, CourseError, CourseValidationResult, CategoryNode, CourseSummary, CourseSection, CourseModuleDetail, MoodlePage, MoodleForum, MoodleBlock, GradeTreeNode, GradeItem, MoodleEnrolledUser, MoodleForumDiscussion } from "./types";
...
export async function getEnrolledUsers(
  ...
  // fetchLimit: large enough to include all teachers even in big courses.
  // We deliberately ignore the server-side roleid option and filter ourselves.
  ...
  // Filter to teacher roles only when the caller requested role 3 (editingteacher).
  // If no roleId filter was requested, return all users as-is.
  ...
  // Honour the caller's limit on the filtered result (default: return all teachers).
