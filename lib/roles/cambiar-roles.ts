...
import { assignRoles, getEnrolledUsersWithRoles, unassignRoles } from "@/lib/moodle/moodle.service";
...
export async function cambiarRolesEnCurso({
  ...
  const leerRoles = async (): Promise<Map<number, Set<number>>> => {
    const usuarios = await getEnrolledUsersWithRoles(moodleUrl, token, courseId);
